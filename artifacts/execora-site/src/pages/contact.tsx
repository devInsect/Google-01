...
export default function Contact() {
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setError('');
    setDraftHref(`mailto:hello@execora.co?subject=${encodeURIComponent(`Execora enquiry from ${name}`)}&body=${encodeURIComponent(`Name: ${name}\nEmail: ${email}\n\n${message}`)}`);
    setSent(true);
        ...
        <div className="container-shell grid gap-12 lg:grid-cols-[.85fr_1.15fr]">
          <div>
            <div className="image-crop aspect-[.9] bg-secondary">
              <img src={contactImage} alt="Execora advisor in a thoughtful client conversation" width="1024" height="1024" loading="eager" decoding="async" className="object-[center_70%]" data-testid="img-contact" />
            </div>
          ...
          <div className="rounded-[1.65rem] bg-secondary p-6 md:p-10">
            ...
            {sent ? (
                ...
                <span className="grid h-12 w-12 place-items-center rounded-full bg-accent text-primary"><Check size={22} /></span>
                  <h2 className="display mt-7 text-4xl font-semibold leading-none">Your note is ready.</h2>
                <p className="mt-5 max-w-sm leading-7 text-muted-foreground">Your note is ready to send in your email app. We will reply within two working days once it arrives.</p>
              ...
              <form onSubmit={handleSubmit} className="mt-8 space-y-6" noValidate>
                ...
                <label className="block"><span className="eyebrow text-primary/55">What is on your mind?</span><textarea name="message" rows={5} required minLength={10} placeholder="Tell us a little about the decision or change you are navigating." className="mt-3 w-full resize-y border-0 border-b border-foreground/20 bg-transparent px-0 py-3 text-base outline-none placeholder:text-muted-foreground/55 focus:border-primary" data-testid="textarea-contact-message" /></label>
